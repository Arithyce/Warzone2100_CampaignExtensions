...
function builtDerrick()
{
	camManageGroup(camMakeGroup("GroupA", ENEMIES), CAM_ORDER_ATTACK, {
		...
		camPlayVideos({video: "C1L1_MSG", type: MISS_MSG});
}
...
{
	camPlayVideos({video: "C1L1_MSG3", type: MISS_MSG});
...
function camEnemyBaseEliminated_scavGroup1()
{
	...
	camPlayVideos({video: "C1L1_MSG4", type: MISS_MSG});
}

function camEnemyBaseEliminated_scavGroup0()
{
	camPlayVideos({video: "C1L1_MSG2", type: MISS_MSG});
...
function eventStartLevel()
{
	...
	camSetStandardWinLossConditions(CAM_VICTORY_STANDARD, "C1L2");
	...
	camPlayVideos({video: "C1IN_MSG", type: CAMP_MSG, immediate: false});
	if (difficulty === HARD)
	{
	...
	camSetEnemyBases({
		"scavGroup1": {
			cleanup: "PlayerBase",
			detectMsg: "C1L1_POST0",
			detectSnd: "pcv375.ogg",
			eliminateSnd: "pcv391.ogg"
		},
		"scavGroup2": {
			cleanup: "ScavBase",
			detectMsg: "C1L1_BASE0",
			detectSnd: "pcv374.ogg",
			eliminateSnd: "pcv392.ogg"
		},
		"scavGroup0": {
			cleanup: "ScavBase0",
			detectMsg: "C1L1_BASE1",
			detectSnd: "pcv374.ogg",
			eliminateSnd: "pcv392.ogg"
		},
	});
	...
	camSetFactories({
		"ScavFac": {
			...
			templates: [
			cTempl.bloketwin, cTempl.triketwin, cTempl.buggytwin, cTempl.bjeeptwin, cTempl.buscan, cTempl.firecan, cTempl.firetruck ]
		},
		"ScavFac0": {
			assembly: "Assembly0",
			order: CAM_ORDER_ATTACK,
